...
public class AutoFillSugar {
    ...
    func setNextValidSequenceNumber(
        ...
        guard let response = try? await client.request(request) else {
            debugPrint("no account info data")
            return
        }
        
        tx["Sequence"] = response.accountData.sequence as AnyObject
    }

    // TODO: will decide whether uncomment, keep these lines or remove them
...
//    }
    ...
    func checkAccountDeleteBlockers(
        ...
        guard let response = try? await client.request(request) else {
            debugPrint("no account object data")
            return
        }
        
        if response.accountObjects.count > 0 {
            throw XrplError(
