...
internal struct PrivateKey {
    ...
    internal init(seed: Data, coin: Coin) {
        let output = try! Data(CryptoSwift.HMAC(key: "Bitcoin seed".data(using: .ascii)!.bytes, variant: .sha2(.sha512)).authenticate(seed.bytes))
    ...
    internal var publicKey: Data {
        ...
        guard let masterPublicKey = try? SECP256K1.derivePublicKey(ctx: ctx, secretKey: _data.getPointer()) else {
            return Data()
        }
    ...
    internal func derived(at node: DerivationNode) throws -> PrivateKey {
        ...
        case .notHardened:
            var _data = raw
            let ctx = secp256k1_context_create(UInt32(SECP256K1_CONTEXT_SIGN))!
            guard let pk = try? SECP256K1.derivePublicKey(ctx: ctx, secretKey: _data.getPointer()) else {
                throw PrivateKeyError.invalidPublicKey
            }
        ...
        guard let digest = try? Data(HMAC.init(key: chainCode.bytes, variant: .sha2(.sha512)).authenticate(data.bytes)) else {
            throw PrivateKeyError.cannotHash
        }
