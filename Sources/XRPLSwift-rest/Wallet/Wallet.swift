...
public class Wallet {
     ...
     */
    public static func fromMnemonics(
        _ mnemonic: String,
        ...
        // m/44'
        let purpose = try node.derived(at: .hardened(44))
        // m/44'/144'
        let coinType = try purpose.derived(at: .hardened(144))
        // m/44'/144'/0'
        let account = try coinType.derived(at: .hardened(opts.derivationPath.account))
        // m/44'/144'/0'/0
        let change = try account.derived(at: .notHardened(opts.derivationPath.change))
        // m/44'/144'/0'/0/0
        let firstPrivateKey = try change.derived(at: .notHardened(opts.derivationPath.addressIndex))
    ...
    // TODO: will active the lines soon
//
//    /**
